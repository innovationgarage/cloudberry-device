...
chi_login_url() {
	...
	pap)
	    ...
	    echo -n "http://192.168.182.1:$HS_UAMPORT/login?username=$1&password=${response}&userurl=${3:-$COOVA_USERURL}"
	    ;;
	mschapv2)
	    response=$($CHILLI_RESPONSE -nt "$CHI_CHALLENGE" "$HS_UAMSECRET" "$1" "$2")
	    echo -n "http://192.168.182.1:$HS_UAMPORT/login?username=$1&ntresponse=${response}&userurl=${3:-$COOVA_USERURL}"
	    ;;
	*)
	    response=$($CHILLI_RESPONSE "$CHI_CHALLENGE" "$HS_UAMSECRET" "$2")
	    echo -n "http://192.168.182.1:$HS_UAMPORT/login?username=$1&response=${response}&userurl=${3:-$COOVA_USERURL}"
	    ;;
    esac
}
...
[ "$FORM_res" = "wispr" ] && \
    ...
    http_redirect2 "http://192.168.182.1:$HS_UAMPORT/login?username=$FORM_username&WISPrEAPMsg=$FORM_WISPrEAPMsg&WISPrVersion=2.0"
}
